...
test_bandwidth() {
    ...
    # Start iperf3 server on Site3
    sudo ip netns exec site3 iperf3 -s -D -1 > /dev/null 2>&1
    sleep 2

    # Test HQ to Site3
    print_test "Testing bandwidth HQ → Site3"
    local bw3=$(sudo ip netns exec hq iperf3 -c 10.4.1.10 -t 5 -J 2>/dev/null | grep "bits_per_second" | head -1 | awk -F': ' '{print $2}' | awk -F',' '{print $1}')
    if [ -n "$bw3" ]; then
        local bw3_mbps=$(echo "scale=2; $bw3 / 1000000" | bc)
        print_success "HQ → Site3: ${bw3_mbps} Mbps"
        echo "HQ→Site3: ${bw3_mbps} Mbps" >> "$results_file"
    else
        print_fail "Could not measure bandwidth to Site3"
    fi
